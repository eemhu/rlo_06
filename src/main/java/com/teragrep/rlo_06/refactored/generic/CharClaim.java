...
public final class CharClaim implements Claim<TrackedLease<MemorySegment>> {
    ...
    public Result<TrackedLease<MemorySegment>> advance(final List<TrackedLease<MemorySegment>> src) {
        for (final TrackedLease<MemorySegment> lease : src) {
            if (lease.hasNext()) {
                ...
                if (b == targetChar) {
                    // Claim successful
                    return new ResultImpl<>(ResultName.CHAR, lease.sliceWithLength(lease.currentPosition() - 1, 1));
                }
                else {
                    lease.reset();
                    throw new ClaimFailedException(
                            getClass(),
                            "expected <[" + targetChar + "]> but found " + Character.toString(b)
                    );
                }
            }
        }
